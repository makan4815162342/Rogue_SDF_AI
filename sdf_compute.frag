// Rogue SDF AI — Voxel Compute Fragment Shader (Vertex Attribute Version)

// This now comes from the vertex shader
in vec3 v_voxel_coord;

out vec4 fragColor;

// --- UNIFORMS for coordinate conversion ---
uniform vec3 uBoundsMin;
uniform vec3 uBoundsMax;
uniform int uGridResX;
uniform int uGridResY;
uniform int uGridResZ;
uniform bool uIsVertexLookup;

// --- All other uniforms (uCount, uShapePos, etc.) are IDENTICAL to sdf_volume.frag ---
uniform int    uCount;
...
// --- All SDF primitives, helpers, and combine functions are IDENTICAL to sdf_volume.frag ---
// (This entire block of SDF functions is unchanged)
...
Hit evalPrim(int i, vec3 pW) {
    ...
    else if (st == 4) { sd = sdfCone(lp, S.y, S.x, S.z); }
    else if (st == 2) {
        vec3 AS = abs(S);
        float scale_factor = min(AS.x, min(AS.y, AS.z));
        lp /= AS;
...
float opPipe(float a, float b, float r) { return opRoundI(a, b, r * 0.5) - r; }
float opPipeU(float a, float b, float r) { return min(min(a, b), opPipe(a, b, r)); }
float opPipeSub(float a, float b, float r) { return min(max(a, -b), opPipe(a, b, r)); }
float opPipeI(float a, float b, float r) { return opPipe(a, b, r); }
Hit combine(Hit A, Hit B, int op, float k, float strength, float fill) {
    ...
    int id;
    float h_col = 0.0;
    if (op == 6) { d = A.d; id = A.id; c = mix(A.col, B.col, clamp(1.0 - db / max(strength, 0.001), 0.0, 1.0)); return Hit(d, c, id); }
    else if (op == 7) { float displacement = strength * (1.0 - smoothstep(0.0, k * strength + 1e-6, db)); d = da - displacement; h_col = clamp(displacement / (abs(strength) + 1e-6), 0.0, 1.0); c = mix(A.col, B.col, h_col); id = A.id; d *= 0.7; return Hit(d, c, id); }
    else if (op == 8) { float displacement = strength * (1.0 - smoothstep(0.0, k * strength + 1e-6, db)); d = da + displacement; h_col = clamp(displacement / (abs(strength) + 1e-6), 0.0, 1.0); c = mix(A.col, B.col, h_col); id = A.id; d *= 0.7; return Hit(d, c, id); }
    else if (op == 9) { float carved_base = opRoundSub(da, db - strength, k); d = min(carved_base, db); if (carved_base < db) { c = A.col; id = A.id; } else { c = B.col; id = B.id; } return Hit(d, c, id); }
    else if (op == 10) { float carved_tool = opRoundSub(db, da - strength, k); d = min(carved_tool, da); if (carved_tool < da) { c = B.col; id = B.id; } else { c = A.col; id = A.id; } return Hit(d, c, id); }
    else if (op == 11) {
    ...
    bool isSmoothBlend = (op >= 3 && op <= 5) || (op >= 23 && op <= 35);
    if (uColorBlendMode == 1 && k > 0.0 && isSmoothBlend) {
        ...
        if (op == 4 || op == 14 || op == 24 || op == 34) { if (da > -db) { c = A.col; id = A.id; } else { c = B.col; id = B.id; } }
        else if (op == 5 || op == 15 || op == 25 || op == 35) { if (da > db) { c = A.col; id = A.id; } else { c = B.col; id = B.id; } }
        else { if (da < db) { c = A.col; id = A.id; } else { c = B.col; id = B.id; } }
    }
    return Hit(d, c, id);
}
Hit sceneSDF(vec3 p) {
...
// ── MAIN ──────────────────────────────────────────────────────────────────
void main() {
    // The voxel coordinate is received directly from the vertex shader
    vec3 voxel_coord = v_voxel_coord;

    // Normalize the coordinate to the [0, 1] range
    vec3 uvw;
    if (uIsVertexLookup) {
        // For vertices, the coords are already floats, just normalize them
        uvw = vec3(voxel_coord.x / float(uGridResX - 1),
                   voxel_coord.y / float(uGridResY - 1),
                   voxel_coord.z / float(uGridResZ - 1));
    } else {
        // For grid indices, add 0.5 to sample the center of the voxel
        uvw = vec3((voxel_coord.x + 0.5) / float(uGridResX),
                   (voxel_coord.y + 0.5) / float(uGridResY),
                   (voxel_coord.z + 0.5) / float(uGridResZ));
    }
    
    // Convert normalized coordinate to world position
    vec3 p = mix(uBoundsMin, uBoundsMax, uvw);
    
    // Evaluate the SDF at that world position
    Hit h = sceneSDF(p);
    
    // Output the result (distance in R, color in GBA)
